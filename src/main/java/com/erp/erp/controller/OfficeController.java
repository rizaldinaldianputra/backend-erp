...
public class OfficeController {
        ...
        @GetMapping
        @Operation(summary = "Get all offices with pagination")
        public ResponseEntity<ApiResponseDto<Page<OfficeResponse>>> getAll(
                        @RequestParam(defaultValue = "0") int page,
                        @RequestParam(defaultValue = "10") int size) {
                Pageable pageable = PageRequest.of(page, size);
                return ResponseEntity.ok(ApiResponseDto.<Page<OfficeResponse>>builder()
                                .status("success")
                                .message("Offices fetched successfully")
                                .data(officeService.findAllWithOrganization(pageable))
                                .build());
        }
